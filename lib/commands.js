...
async function generateBookingList(remoteJid) {
    ...
    const parsedBookings = rawBookings
        .map(b => ({
            ...parseBooking(b.booking),
            bookerJid: b.bookerJid
        }))
        .filter(b => b.endDateTime > now); // Only show future bookings
    ...
    // Assign indices AFTER filtering so they are sequential (1, 2, 3...)
    parsedBookings.forEach((b, i) => b.index = i + 1);
