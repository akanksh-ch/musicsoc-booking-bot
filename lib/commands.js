    ...
    'list': {
        ...
        handler: async (sock, msg, args) => {
            ...
            for (const [header, items] of Object.entries(grouped)) {
                listText += `${header}\n`;
                items.forEach(item => {
                    // Display: TimeRange (Index) or just TimeRange? 
                    // User showed "13:00 - 14:00 @~Charlie". We don't have descriptions yet.
                    // But we need the index for /cancel. 
                    // Let's format as: "13:00 - 14:00 [ID: 1]" so they know what to cancel.
                    listText += `${item.timeRange} [ID: ${item.index}]\n`;
                });
                listText += '\n';
            }
